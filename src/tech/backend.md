...
通过克隆一份内容，借助纯CSS无限循环动画，实现无缝循环滚动。
...
<body>
    <div class="wrap">
        <h1 class="title">纯 CSS 走马灯：两份内容（克隆）实现无缝循环</h1>
        <p class="desc">
            原理：在同一条轨道里放两组完全相同的标签（第二组是第一组的克隆），让轨道从 0 滚到 -50%。
            悬停可暂停。
        </p>
        <div class="marquee">
            <div class="marquee-viewport">
                <div class="marquee-track">
                    ...
                    <span class="chip">写作</span>
                    <span class="chip">函数</span>
                    <span class="chip">几何</span>
                    <span class="chip">概率</span>
                    <span class="chip">方程</span>
                    <span class="chip">语法</span>
                    <span class="chip">词汇</span>
                    <span class="chip">听力</span>
                    <span class="chip">力学</span>
                    <span class="chip">电学</span>
                    <span class="chip">光学</span>
                    <span class="chip">方程式</span>
                    <span class="chip">元素周期</span>
                    <span class="chip">实验</span>
                    <span class="chip">遗传</span>
                    <span class="chip">细胞</span>
                    <span class="chip">生态</span>
                    <span class="chip">近代史</span>
                    <span class="chip">世界史</span>
                    <span class="chip">气候</span>
                    <span class="chip">地形</span>
                    <span class="chip">法治</span>
                    <!-- 第二组（克隆：内容必须与第一组完全一致，才能无缝衔接） -->
                    <span class="chip">阅读理解</span>
                    <span class="chip">写作</span>
                    <span class="chip">函数</span>
                    <span class="chip">几何</span>
...
- [marquee 示例](https://jasonbai008.github.io/marquee/)
- [marquee 仓库](https://github.com/jasonbai008/marquee)
...
- `@echo off` 这一行的作用是关闭命令行窗口的回显功能。也就是说，在运行脚本时，命令行不会显示每一条命令的具体内容，只有命令的输出结果会被显示。这让输出看起来更整洁。
- `pause` 这一行会使程序暂停并等待用户按下任意键。它通常用于让用户看到前面的输出信息，直到他们准备好继续或关闭窗口。

## 项目统一格式化代码

1. 在项目根目录新建`.prettierrc`文件，并添加以下设置：
...
- `DOMContentLoaded事件`在 DOM 树构建完成后触发，也就是在页面所有 DOM 元素都加载完成后触发。这意味着在 DOMContentLoaded 事件触发之前，页面中的图片、音频、视频等资源可能还没有加载完成。因此，**DOMContentLoaded 事件适合处理 DOM 操作相关的代码**，比如修改 DOM 节点等。

- `load事件`则是在页面所有资源（包括图片、音频、视频等）都加载完成后触发。这意味着**在 load 事件触发时，页面中所有的资源都已经加载完成**，可以进行一些需要等待资源加载完成的操作，比如计算页面的高度或者初始化一些插件。

总之，DOMContentLoaded 事件触发的时机比 load 事件早，但是它只能处理 DOM 操作相关的代码；而 load 事件则适合处理需要等待所有资源加载完成后才能进行的操作。
...
1. 登录自己的 163 邮箱
2. 选择 设置>POP3/SMTP/IMAP> 开启服务
3. 开启服务过程中会有弹窗显示授权码，复制出来
   :::

```js
const nodemailer = require("nodemailer");

function sendEmail({ senderEmail, receiverEmail, subject, content, smtp }) {
  // 初始化邮件发送实例
  const transporter = nodemailer.createTransport({
    host: "smtp.163.com", // 163邮箱固定值
    port: 465, // SMTP 端口
    auth: {
      user: senderEmail, // 发送者邮箱, same as mailOptions.from
      pass: smtp, // 发送者邮箱的SMTP授权码
    },
  });
  // 实际发送逻辑
...
// 修改以下各个字段：
let data = {
  senderEmail: "youremail@163.com", // 发送者的邮箱
  receiverEmail: "example@163.com", // 接收者的邮箱
  subject: "主题", // 邮件主题
  content: `邮件内容`, // 邮件内容
  smtp: `XXXXXXXXXXXXXXXX`, // 发送者邮箱SMTP授权码
};
...
UUID 是通用唯一识别码，形式为 8-4-4-4-12，每个 x 是一个 16 进制数  
